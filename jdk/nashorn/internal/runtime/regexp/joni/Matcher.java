...
extends IntHolder {
    ...
    public Matcher(Regex regex, Region region, char[] chars) {
        this(regex, region, chars, 0, chars.length);
    }

    public Matcher(Regex regex, Region region, char[] chars, int p, int end) {
        this.regex = regex;
        this.chars = chars;
        this.str = p;
        this.end = end;
    ...
    protected abstract int matchAt(int var1, int var2, int var3, boolean var4);
    ...
    public final int match(int at, int range, int option) {
        ...
        return this.matchAt(range, at, prev, true);
    }
    ...
    private boolean forwardSearchRange(char[] ch, int string, int e, int s, int range, IntHolder lowPrev) {
        ...
        block4: while ((p = this.regex.forward.search(this.regex, ch, p, e, range)) != -1 && p < range) {
    ...
    private boolean backwardSearchRange(char[] ch, int string, int e, int s, int range, int adjrange) {
        ...
        r += this.regex.dMin;
        int p = s;
        block4: while ((p = this.regex.backward.search(this.regex, ch, r, adjrange, e, p, s, r)) != -1) {
            if (this.regex.subAnchor != 0) {
                switch (this.regex.subAnchor) {
                    case 2: {
                        int prev;
    ...
    private boolean matchCheck(int upperRange, int s, int prev, boolean interrupt) {
        return this.matchAt(this.end, s, prev, interrupt) != -1 && !Option.isFindLongest(this.regex.options);
    }
     ...
     */
    public final int search(int startp, int rangep, int option, boolean interrupt) {
        int prev;
        int start = startp;
        int range = rangep;
        int origStart = start;
        ...
        do {
            if (!this.matchCheck(origStart, s, prev = EncodingHelper.prevCharHead(this.str, s), interrupt)) continue;
            return this.match(s);
        } while ((s = prev) >= range);
        return this.mismatch();
    }
