...
implements RegexState {
    ...
    SearchAlgorithm.Forward forward;
    ...
    public Matcher matcher(char[] chars, int p, int end) {
        ...
        return matcherFactory.create(this, this.numMem == 0 ? null : new Region(this.numMem + 1), chars, p, end);
    }

    public Matcher matcherNoRegion(char[] chars) {
        return this.matcherNoRegion(chars, 0, chars.length);
    }

    public Matcher matcherNoRegion(char[] chars, int p, int end) {
        return this.factory.create(this, null, chars, p, end);
    }
    ...
    void setExactInfo(OptExactInfo e) {
        ...
        if (e.ignoreCase) {
            this.forward = SearchAlgorithm.SLOW_IC_FORWARD;
        } else if (e.length >= 2) {
            this.setupBMSkipMap();
            this.forward = SearchAlgorithm.BM_FORWARD;
        } else {
            this.forward = SearchAlgorithm.SLOW_FORWARD;
        }
        this.backward = SearchAlgorithm.SLOW_BACKWARD;
        this.dMin = e.mmd.min;
        this.dMax = e.mmd.max;
    ...
    void setOptimizeMapInfo(OptMapInfo m) {
        this.map = m.map;
        this.forward = SearchAlgorithm.MAP_FORWARD;
        this.backward = SearchAlgorithm.MAP_BACKWARD;
        this.dMin = m.mmd.min;
        this.dMax = m.mmd.max;
        if (this.dMin != Integer.MAX_VALUE) {
    ...
    public String optimizeInfoToString() {
        ...
        s.append("optimize: ").append(this.forward.getName()).append("\n");
        ...
        } else if (this.forward == SearchAlgorithm.MAP_FORWARD) {
            int n = 0;
            for (int i = 0; i < 256; ++i) {
